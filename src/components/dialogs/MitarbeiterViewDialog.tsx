    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Beschäftigungsumfang</Label>
            <Badge variant="secondary">{record.fields.beschaeftigungsumfang?.label ?? '—'}</Badge>
          </div>
          <div className="space-y-1">
          ...
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Verfügbare Wochentage</Label>
            <p className="text-sm">{Array.isArray(record.fields.verfuegbare_tage) ? record.fields.verfuegbare_tage.map((v: any) => v?.label ?? v).join(', ') : '—'}</p>
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Anmerkungen</Label>
            <p className="text-sm whitespace-pre-wrap">{record.fields.anmerkungen_mitarbeiter ?? '—'}</p>
          </div>
        </div>
      </DialogContent>
    </Dialog>
