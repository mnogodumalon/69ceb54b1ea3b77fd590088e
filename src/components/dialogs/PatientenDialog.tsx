...
export function PatientenDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: PatientenDialogProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'patienten');
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      const schema = `{\n  "patient_vorname": string | null, // Vorname\n  "patient_nachname": string | null, // Nachname\n  "geburtsdatum": string | null, // YYYY-MM-DD\n  "pflegegrad": LookupValue | null, // Pflegegrad (select one key: "pg1" | "pg2" | "pg3" | "pg4" | "pg5") mapping: pg1=Pflegegrad 1, pg2=Pflegegrad 2, pg3=Pflegegrad 3, pg4=Pflegegrad 4, pg5=Pflegegrad 5\n  "strasse": string | null, // Straße\n  "hausnummer": string | null, // Hausnummer\n  "plz": string | null, // Postleitzahl\n  "ort": string | null, // Ort\n  "telefon_patient": string | null, // Telefonnummer\n  "leistungsarten": LookupValue[] | null, // Benötigte Leistungsarten (select one or more keys: "grundpflege" | "behandlungspflege" | "hauswirtschaft" | "betreuung" | "medikamentengabe" | "wundversorgung" | "mobilisation") mapping: grundpflege=Grundpflege, behandlungspflege=Behandlungspflege, hauswirtschaft=Hauswirtschaft, betreuung=Betreuung, medikamentengabe=Medikamentengabe, wundversorgung=Wundversorgung, mobilisation=Mobilisation\n  "hinweise_patient": string | null, // Besondere Hinweise\n}`;
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="patient_vorname">Vorname</Label>
            <Input
              id="patient_vorname"
              value={fields.patient_vorname ?? ''}
              onChange={e => setFields(f => ({ ...f, patient_vorname: e.target.value }))}
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="patient_nachname">Nachname</Label>
            <Input
              id="patient_nachname"
              value={fields.patient_nachname ?? ''}
              onChange={e => setFields(f => ({ ...f, patient_nachname: e.target.value }))}
            />
          </div>
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            <Label htmlFor="standort">Standort auf Karte</Label>
          ...
          <div className="space-y-2">
            <Label htmlFor="telefon_patient">Telefonnummer</Label>
            <Input
              id="telefon_patient"
              value={fields.telefon_patient ?? ''}
              onChange={e => setFields(f => ({ ...f, telefon_patient: e.target.value }))}
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="leistungsarten">Benötigte Leistungsarten</Label>
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <Checkbox
                  id="leistungsarten_grundpflege"
                  checked={lookupKeys(fields.leistungsarten).includes('grundpflege')}
                  onCheckedChange={(checked) => {
                    setFields(f => {
                      const current = lookupKeys(f.leistungsarten);
                      const next = checked ? [...current, 'grundpflege'] : current.filter(k => k !== 'grundpflege');
                      return { ...f, leistungsarten: next.length ? next as any : undefined };
                    });
                  }}
                />
                <Label htmlFor="leistungsarten_grundpflege" className="font-normal">Grundpflege</Label>
              </div>
              <div className="flex items-center gap-2">
                <Checkbox
                  id="leistungsarten_behandlungspflege"
                  checked={lookupKeys(fields.leistungsarten).includes('behandlungspflege')}
                  onCheckedChange={(checked) => {
                    setFields(f => {
                      const current = lookupKeys(f.leistungsarten);
                ...
                />
                <Label htmlFor="leistungsarten_behandlungspflege" className="font-normal">Behandlungspflege</Label>
              </div>
              <div className="flex items-center gap-2">
                <Checkbox
                  id="leistungsarten_hauswirtschaft"
                  checked={lookupKeys(fields.leistungsarten).includes('hauswirtschaft')}
                  onCheckedChange={(checked) => {
                    setFields(f => {
                      const current = lookupKeys(f.leistungsarten);
                ...
                />
                <Label htmlFor="leistungsarten_hauswirtschaft" className="font-normal">Hauswirtschaft</Label>
              </div>
              <div className="flex items-center gap-2">
                <Checkbox
                  id="leistungsarten_betreuung"
                  checked={lookupKeys(fields.leistungsarten).includes('betreuung')}
                  onCheckedChange={(checked) => {
                    setFields(f => {
                      const current = lookupKeys(f.leistungsarten);
                ...
                />
                <Label htmlFor="leistungsarten_betreuung" className="font-normal">Betreuung</Label>
              </div>
              <div className="flex items-center gap-2">
                <Checkbox
                  id="leistungsarten_medikamentengabe"
                  checked={lookupKeys(fields.leistungsarten).includes('medikamentengabe')}
                  onCheckedChange={(checked) => {
                    setFields(f => {
                      const current = lookupKeys(f.leistungsarten);
                ...
                />
                <Label htmlFor="leistungsarten_medikamentengabe" className="font-normal">Medikamentengabe</Label>
              </div>
              <div className="flex items-center gap-2">
                <Checkbox
                  id="leistungsarten_wundversorgung"
                  checked={lookupKeys(fields.leistungsarten).includes('wundversorgung')}
                  onCheckedChange={(checked) => {
                    setFields(f => {
                      const current = lookupKeys(f.leistungsarten);
                      const next = checked ? [...current, 'wundversorgung'] : current.filter(k => k !== 'wundversorgung');
                      return { ...f, leistungsarten: next.length ? next as any : undefined };
                    });
                  }}
                />
                <Label htmlFor="leistungsarten_wundversorgung" className="font-normal">Wundversorgung</Label>
              </div>
              <div className="flex items-center gap-2">
                <Checkbox
                  id="leistungsarten_mobilisation"
                  checked={lookupKeys(fields.leistungsarten).includes('mobilisation')}
                  onCheckedChange={(checked) => {
                    setFields(f => {
                      const current = lookupKeys(f.leistungsarten);
                ...
                />
                <Label htmlFor="leistungsarten_mobilisation" className="font-normal">Mobilisation</Label>
              </div>
            </div>
          </div>
          <div className="space-y-2">
            <Label htmlFor="hinweise_patient">Besondere Hinweise</Label>
            <Textarea
              id="hinweise_patient"
              value={fields.hinweise_patient ?? ''}
              onChange={e => setFields(f => ({ ...f, hinweise_patient: e.target.value }))}
              rows={3}
            />
          </div>
          <DialogFooter>
